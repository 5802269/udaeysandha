...
void loop(){
  ...
  //Drive Forward Until Aligned With First Opening
  pingUltraS();
  totalDistancO = totalDistance;
  while(true){
    driveStraight(1,100);
    pingUltraS();
    if(totalDistance > totalDistancO + 10){
      break;
    }
  }

  //Turn Right (Toward First Tree)
  turnLeft(90,100);

  //Drive Forward Until Wall -> Friction To Pick Up Tree
  driveStraight(100, 180);

  //Raise Tree Up

  //Realign With Opening From Tree Pick Up Location
  reAlignToOpening();

  //Nagivate Through Maze From Perpendicular To The Opening
  navigateMaze();
}



int reAlignToOpening(){
 
}

int navigateMaze(){
  while(true){ //If UltraSonic Sensors Distance Stop Changing -> Drop Trees And Stop
   //360 Degree Right Arched Turn
   //Turn Correction
   //Forward
   //90 Degree Left Turn
   //Turn Correction
...
int turnCorrection(int recursion = 2) {
  //Store Inital Distances
  pingUltraS();
  leftDistanceO = leftDistance;
  rightDistanceO = rightDistance;
  totalDistanceO = rightDistance + leftDistance;
  ...
  pingUltraS();
  leftDistanceDiff = leftDistance - leftDistanceO;
  rightDistanceDiff = rightDistance - rightDistanceO;
  totalDistanceF = rightDistance + leftDistance;
  

  //If Not Straight, Call Function Again
  if(leftDistanceDiff > 1){
    turnRight(1,100);
    turnCorrection(recursion + 1);
  }else if(leftDistanceDiff < -1){
    turnLeft(1,100);
    turnCorrection(recursion + 1);
  }else if(leftDistanceDiff > -1 && leftDistanceDiff < 1){
